...
echo "==> Switching .env.production to us-east-1 (dev)"
sed -i '' \
...
echo "==> 2. Prebuild mobile (dev)"
cd "$SCRIPT_DIR/nora-mobile" && npx expo prebuild --platform ios

echo "==> 3. Install pods"
cd "$SCRIPT_DIR/nora-mobile/ios" && pod install

echo "==> 4. Deploy backend"
cd "$SCRIPT_DIR" && ./docker_deploy.sh
