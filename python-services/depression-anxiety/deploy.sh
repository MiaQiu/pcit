#!/bin/bash
# Deploy the DAM (depression/anxiety) Lambda to the same ECR account as App Runner.
# First run: set LAMBDA_ROLE_ARN to an IAM role with AmazonS3ReadOnlyAccess.
# The existing nora-diarization-role works as-is — same permissions needed.
# e.g. LAMBDA_ROLE_ARN=arn:aws:iam::059364397483:role/nora-diarization-role ./deploy.sh
set -e

ACCOUNT_ID=059364397483
REGION=ap-southeast-1
ECR_REPO=nora-depression-anxiety
FUNCTION_NAME=nora-depression-anxiety
...
echo "  DAM_LAMBDA_NAME=${FUNCTION_NAME}"
