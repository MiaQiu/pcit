...
| `scripts/_tmp_radar_chart.cjs` | Debug script: plot radar chart for any child from prod DB |

## Debug Script

`scripts/_tmp_radar_chart.cjs` queries the production database directly and generates a standalone HTML radar chart for a given child ID. Useful for support, QA, and manual developmental reviews.

### Prerequisites

The prod DB tunnel must be running on `localhost:5433` before executing the script:

```bash
# Terminal 1 — keep this running
bash scripts/start-prod-db-tunnel.sh
```

### Usage

1. Open `scripts/_tmp_radar_chart.cjs` and set `CHILD_ID` at the top to the target child's UUID:

```js
const CHILD_ID = 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx';
```

2. Run the script:

```bash
node scripts/_tmp_radar_chart.cjs
```
...
```
Child ID:  a44fa924-caa9-453b-936a-0ea0291eaf7f
Age:       45 months (3.8 years)

───────────────────────────────────────────────────────────────────────────
Domain       Achieved   Emerging   Total    Benchmark    Child%   Bench%
───────────────────────────────────────────────────────────────────────────
Language     9          1          11       9.8            86.4%    89.1%
Cognitive    5          0          5        2.5           100.0%    50.0%
Social       3          2          5        2.75           80.0%    55.0%
Emotional    2          1          3        1.38           83.3%    46.0%
Connection   2          0          2        0.88          100.0%    44.0%
───────────────────────────────────────────────────────────────────────────
```

**Browser** — saves and opens `scripts/_tmp_radar_chart_output.html` with:
- SVG radar chart (purple filled polygon = child, orange dashed = benchmark)
- Per-domain table with ▲ ahead / ▼ below status
- Child age subtitle

### How Child % is calculated

```
childValue = (achieved + emerging * 0.5) / total * 100
```

Emerging milestones count as 50% — matches the weighting in `RadarChart.tsx`.

### How Benchmark % is calculated

Same interpolation logic as `calculateBenchmark()` in `server/routes/learning.cjs`:

- Walk through each stage in the domain, sorted by start age
- **Completed stage** (`childAge >= stage.end`): add all milestones in the stage
- **Current stage** (`stage.start <= childAge < stage.end`): add `stageCount × (childAge - start) / (end - start)`
- **Future stage** (`childAge < stage.start`): add nothing
- Result divided by `total` to get a percentage

### Notes

- The prod DB table is `milestone_library` with snake_case columns (`grouping_stage`, `median_age_months`, etc.) — the Prisma model name `MilestoneLibrary` does **not** match the raw table name
- The script uses `Child.birthday` first, falls back to `User.childBirthday`, then `User.childBirthYear` (approximate, mid-year offset)
- Child ID not found → the UUID may be a User ID, not a Child ID; or the record may be in a different environment
