/**
 * WACB Question 10 Screen — the final Child Snapshot item. Submits the whole
 * survey to the backend before navigating on.
 */
...
export const WacbQuestion10Screen: React.FC = () => {
  ...
  const handleSubmitSurvey = async (selectedValue: number, _updateData: any, navigation: any) => {
    // Submit all Child Snapshot data to the backend
    try {
      const API_URL = process.env.EXPO_PUBLIC_API_URL || 'http://localhost:3001';
      const wacbData = {
        ...data.wacb,
        q10LieSteal: selectedValue,
        parentingStressLevel: data.wacb?.parentingStressLevel || 1, // Default value if not provided
      };

      console.log('Submitting Child Snapshot survey to:', `${API_URL}/api/wacb-survey`);
  ...
  return (
    <MultipleChoiceScreen
      {...getWacbQuestions(childName, t)[9]}
