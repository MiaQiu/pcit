...
export default function OnboardingLayout({ children, className = '', progress, onBack, backTo }: OnboardingLayoutProps) {
  return (
    <div className="h-screen bg-white flex flex-col items-center justify-start overflow-hidden">
      <div className={`w-full max-w-[480px] h-full flex flex-col overflow-hidden ${className}`}>
        <div className="pt-8 pb-3 text-center">
          <span className="text-4xl font-bold tracking-[0.12em] text-[#1E2939] uppercase">Nora</span>
        </div>
        {progress !== undefined && (
